...
class SquadPaymentService:
    ...
    def initiate_payment(self, email, amount_naira, transaction_ref, callback_url, customer_name=''):
        ...
        try:
            ...
            data = response.json()
            logger.info(f'Squad initiate response: status={response.status_code}, ref={transaction_ref}')
    ...
    def verify_payment(self, transaction_ref):
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f'Squad verify response not JSON: status={response.status_code}, '
                    f'content={response.text[:200]}'
                )
                raise SquadPaymentError(
                    f'Squad returned invalid response (Status {response.status_code}): '
                    f'{response.text[:100]}'
                )
